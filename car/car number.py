...
# 读取待检测图片
origin_image = cv2.imread('E:/cars/P11.jpg')
...
for item in contours:
    ...
    if (weight > (height * 3.5)) and (weight < (height * 4.5)):
...
def get_chinese_words_list():
    ...
    for i in range(34, 65):
        #将模板存放在字典中
        c_word = read_directory('E:/refer1/'+ template[i])
...
def get_eng_words_list():
    ...
    for i in range(10, 34):
        e_word = read_directory('E:/refer1/' + template[i])
...
def get_eng_num_words_list():
    eng_num_words_list = []
    for i in range(0, 34):
        word = read_directory('E:/refer1/' + template[i])
